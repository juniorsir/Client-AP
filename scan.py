...
class PrinterScanner:
    ...
    def is_host_up(self, ip):
        """
        Ping check to avoid waiting on dead IPs.
        Returns True if host responds.
        """
        param = '-n' if self.os_type == 'Windows' else '-c'
        timeout_arg = '-w' if self.os_type == 'Windows' else '-W'
        timeout_val = '200' if self.os_type == 'Windows' else '0.2' # ms for Win, s for Linux
        
        cmd = ['ping', param, '1', timeout_arg, timeout_val, str(ip)]
        try:
    ...
    def scan_host(self, ip):
        ...
        if open_ports:
            ...
            print(f"{GREEN}[FOUND]{NC} {ip_str.ljust(15)} | {CYAN}{name[:35].ljust(35)}{NC} | Ports: {len(open_ports)}")
    ...
    def run(self):
        ...
        if not net:
            print(f"{RED}[ERROR] Could not determine local network.{NC}")
            return

        print(f"\n{BOLD}Scanning Network: {YELLOW}{net}{NC}")
        print(f"{CYAN}Methods: Ping -> Ports -> SNMP/IPP/PJL/HTTP{NC}\n")
        
        with ThreadPoolExecutor(max_workers=60) as executor:
            results = executor.map(self.scan_host, net.hosts())
            for r in results:
...
# ACTION FUNCTIONS
# ==========================================
def print_universal_test_page(ip):
    print(f"\n{CYAN}[PRINTING]{NC} Sending Polyglot Test Page to {ip}...")
    
    # "Polyglot" Payload:
    # 1. UEL (Universal Exit Language) -> Resets parser
    # 2. PJL -> Enters PCL mode
    # 3. Text Data
    # 4. Form Feed (\f) -> Ejects page
    # 5. UEL -> Ends Job
    payload = (
        b"\x1B%-12345X@PJL\r\n"
    ...
    try:
        with socket.create_connection((ip, 9100), timeout=5) as s:
            s.sendall(payload)
        print(f"{GREEN}[SUCCESS]{NC} Data sent to Port 9100.")
    ...
    while True:
        ...
        if choice == "1":
            if 9100 in target['ports']:
                print_universal_test_page(target['ip'])
            else:
