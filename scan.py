...
def print_universal_test_page(scanner_instance, ip):
    print(f"\n{BOLD}--- Starting PDF Print Job on {ip} ---{NC}")

    # STEP 1: Pre-Flight Check
    print(f"{YELLOW}[STEP 1]{NC} Checking printer health...", end=' ')
    code, msg = scanner_instance.get_detailed_status(ip)
    if code == 6: 
        print(f"\n{RED}[STOP]{NC} Printer error: {BOLD}{msg}{NC}")
        if input(f"{YELLOW}Try anyway? (y/n): {NC}").lower() != 'y': return
    else:
        print(f"{GREEN}OK ({msg}){NC}")

    # STEP 2: Prepare a Real PDF (Like Android does)
    print(f"{YELLOW}[STEP 2]{NC} Generating Minimal PDF...")
    
    # This is a Base64 encoded "Hello World" PDF file
    # It allows us to send a real file without needing external libraries
    pdf_base64 = (
        "JVBERi0xLjcKCjEgMCBvYmogICUgZW50cnkgcG9pbnQKPDwKICAvVHlwZSAvQ2F0YWxvZwog"
        "IC9QYWdlcyAyIDAgUgo+PgplbmRvYmoKCjIgMCBvYmogICUgcGFnZXM9MQo8PAogIC9UeXBl"
        "IC9QYWdlcwogIC9LaWRzIFsgMyAwIFIgXQogIC9Db3VudCAxCj4+CmVuZG9iagoKMyAwIG9i"
        "aiAgJSBwYWdlIDEKPDwKICAvVHlwZSAvUGFnZQogIC9QYXJlbnQgMiAwIFIKICAvTWVkaWFC"
        "b3ggWyAwIDAgNTAwIDIsMDBdCiAgL0NvbnRlbnRzIDQgMCBSCj4+CmVuZG9iagoKNCAwIG9i"
        "aiAgJSBjb250ZW50Cjw8CiAgL0xlbmd0aCA0NAo+PgpzdHJlYW0KQlQKL0YxIDI0IFRmCjEw"
        "MCAxMDAgVGQKKFRlcm11eCBQREYgVGVzdCkgVGoKRVQKZW5kc3RyZWFtCmVuZG9iagoKeHJl"
        "ZgowIDUKMDAwMDAwMDAwMCA2NTUzNSBmIAowMDAwMDAwMDEwIDAwMDAwIG4gCjAwMDAwMDAw"
        "NjAgMDAwMDAgbiAgCjAwMDAwMDAxNTcgMDAwMDAgbiAgCjAwMDAwMDAyNTUgMDAwMDAgbiAg"
        "CnRyYWlsZXIKPDwKICAvU2l6ZSA1CiAgL1Jvb3QgMSAwIFIKPj4Kc3RhcnR4cmVmCjM0OQol"
        "JUVPRgo="
    )
    ...
    try:
        ...
        print(f"{RED}[FAIL]{NC} PDF Generation error.")
        return

    # STEP 3: Send via Port 9100 (Most M-series support PDF via 9100)
    print(f"{YELLOW}[STEP 3]{NC} Sending PDF Binary to Port 9100...", end=' ')
    
    try:
        with socket.create_connection((ip, 9100), timeout=5) as s:
            s.sendall(pdf_data)
        print(f"{GREEN}[SENT]{NC}")
        print(f"{CYAN}[INFO]{NC} This mimics Android's behavior (Sending a document, not text).")
    except Exception as e:
        print(f"{RED}[FAIL]{NC} {e}")

    # STEP 4: Verification
    print(f"{YELLOW}[STEP 4]{NC} Waiting 5s for processing...")
    import time
    time.sleep(5)
    
    new_code, new_msg = scanner_instance.get_detailed_status(ip)
    if new_code == 4:
        print(f"{GREEN}[SUCCESS]{NC} Printer accepted the PDF and is printing!")
    else:
        print(f"{MAGENTA}[STATUS]{NC} Printer Status: {new_msg}")
