...
def print_universal_test_page(scanner_instance, ip):
    """
    Robust printing with Pre-checks, Sending, and Post-checks.
    Requires scanner_instance to check SNMP status.
    """
    ...
    if code == 6: # Error State
        print(f"\n{RED}[STOP]{NC} Printer reports a physical error: {BOLD}{msg}{NC}")
        print(f"       Check for Paper Jams, Empty Trays, or Open Doors.")
        if input(f"{YELLOW}Try to print anyway? (y/n): {NC}").lower() != 'y':
            return
    elif code == 4:
        print(f"\n{YELLOW}[WARN]{NC} Printer is already busy printing.")
    else:
        print(f"{GREEN}OK ({msg}){NC}")

    # STEP 2: Connection Check
    print(f"{YELLOW}[STEP 2]{NC} Connecting to Raw Port 9100...", end=' ')
    sock = None
    try:
        sock = socket.create_connection((ip, 9100), timeout=4)
        print(f"{GREEN}Connected.{NC}")
    except socket.timeout:
        print(f"\n{RED}[FAIL]{NC} Connection Timed Out. Printer might be sleeping deeply.")
        return
    except ConnectionRefusedError:
        print(f"\n{RED}[FAIL]{NC} Connection Refused. Port 9100 is closed on this device.")
        return
    except Exception as e:
        print(f"\n{RED}[FAIL]{NC} Network Error: {e}")
        return

    # STEP 3: Data Transmission
    print(f"{YELLOW}[STEP 3]{NC} Sending Data...", end=' ')
    payload = (
        b"\x1B%-12345X@PJL\r\n"
        b"@PJL ENTER LANGUAGE=PCL\r\n"
        b"\r\n"
        b"--------------------------------------\r\n"
        b"   NETWORK PRINTER DIAGNOSTIC TEST\r\n"
        b"--------------------------------------\r\n"
        b"Status:    CONNECTION SUCCESSFUL\r\n"
        b"Target IP: " + ip.encode() + b"\r\n"
        b"Protocol:  RAW / Port 9100\r\n"
        b"Check:     Robust Mode Enabled\r\n"
        b"--------------------------------------\r\n"
        b"\f"
        b"\x1B%-12345X"
    )
    ...
    try:
        ...
        sock.close()
        print(f"{GREEN}Sent successfully.{NC}")
    except Exception as e:
        print(f"\n{RED}[FAIL]{NC} Data transmission cut off: {e}")
        return

    # STEP 4: Post-Print Verification (The "Sent but not printed" check)
    print(f"{YELLOW}[STEP 4]{NC} Verifying acceptance (Waiting 3s)...", end=' ')
    
    # Wait for printer to wake up and process
    import time
    time.sleep(2) 
    
    new_code, new_msg = scanner_instance.get_detailed_status(ip)
    
    if new_code == 4: # Status 4 means 'Printing'
        print(f"\n{GREEN}[SUCCESS]{NC} Printer status changed to: {BOLD}Printing{NC}")
        print(f"          The job was accepted and is processing.")
    elif new_code == 6:
        print(f"\n{RED}[ERROR]{NC} Printer went into Error State immediately!")
        print(f"         It might have jammed or rejected the paper size.")
    elif new_code == 3 and code == 3:
        # It was Idle before, and it is still Idle now.
        print(f"\n{MAGENTA}[WARNING]{NC} Printer status is still '{new_msg}'.")
        print(f"          {BOLD}Potential Issue:{NC} The printer might have silently discarded the data.")
        print(f"          Causes: Driver mismatch (PCL vs PostScript) or Authentication required.")
    else:
        print(f"{GREEN}OK.{NC} Current Status: {new_msg}")
